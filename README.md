...
#### 进化迭代
1. **选择 (Selection)**：锦标赛选择算法，选择适应度最高的个体
2. **交叉 (Crossover)**：LLM驱动的代码语义融合，结合两个父代的优化特征
3. **变异 (Mutation)**：多智能体随机变异，针对特定优化方向进行调整
4. **评估 (Evaluation)**：三级级联评估，计算适应度分数
5. **更新 (Update)**：精英保留策略，保证最优个体不丢失
